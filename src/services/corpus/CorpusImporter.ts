...
export class CorpusImporter {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      try {
        ...
        const newQuestion = await Question.create({
          ...
          correctOptionIndex: validated.respuesta_correcta, explanation: validated.explicacion, tags: validated.tags, contentHash, originImportId: corpusImport._id, active: true
        });
  ...
  static async importFromCsv(userId: string, tenantId: string, fileName: string, csvContent: string) {
    ...
    const mappedData = parseResult.data.map((row) => ({
      ...
      modulo: row.modulo, fuente: row.fuente, tags: row.tags ? String(row.tags).split(',').map(t => t.trim()) : []
    }));
