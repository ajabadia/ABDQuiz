 ...
 *
 * Se ejecuta UNA VEZ antes de todos los tests, incluso antes del webServer.
 * Verifica:
...
async function globalSetup() {
  ...
  if (fs.existsSync(ENV_PATH)) {
    ...
    checkFailed('.env.local', 'Crea ABDQuiz/.env.local (ver docs en playwright.config.ts)');
    allCriticalPassed = false;
  }
  ...
  if (missing.length === 0) {
    ...
    checkFailed(
      `Faltan: ${missing.join(', ')}`,
      `Agrégalas a ABDQuiz/.env.local (sin comillas, sin espacios alrededor de =)`
    );
    allCriticalPassed = false;
  }
  ...
  // Usamos TCP en vez de HTTP para evitar falsos negativos si
  // Next.js no tiene ruta raíz (devuelve 404, el server está vivo igual)
  ...
  if (authReachable) {
    ...
    warn(
      ...
      `Los tests de rutas públicas (/, /logout-success) funcionarán.\n` +
  ...
  if (nodeModulesExist) {
    ...
    checkFailed('node_modules no encontrado', 'Ejecuta pnpm install en ABDQuiz/');
    allCriticalPassed = false;
  }
